...
export const get = asyncHandler(async (req, res, next) => {
  ...
  const data = await DepartmentService.get(id, filter);
  sendResponse(httpStatus.OK, res, data, "Record fetched successfully");
});

export const create = asyncHandler(async (req, res, next) => {
  const data = req.body;
  const createdData = await DepartmentService.create(data);
  sendResponse(
    httpStatus.CREATED,
    res,
    createdData,
...
export const update = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  const data = req.body;
  const updatedData = await DepartmentService.update(id, data);
...
export const deleteData = asyncHandler(async (req, res, next) => {
  const { id } = req.params;
  await DepartmentService.deleteData(id);
